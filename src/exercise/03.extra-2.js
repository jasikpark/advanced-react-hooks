...
function PokemonCacheProvider({children}) {
  ...
  return <PokemonCacheContext.Provider value={value} children={children} />
}
...
function usePokemonCache() {
  ...
  if (!context) {
    throw new Error(
      'Please use `usePokemonCache` inside a `<PokemonCacheProvider/>`',
    )
  }
  return context
}
    ...
    if (!pokemonName) {
      ...
      run(
        fetchPokemon(pokemonName).then(pokemonData => {
          // dispatch is adding data to the cache via pokemonCacheReducer
          dispatch({type: 'ADD_POKEMON', pokemonName, pokemonData})
          // returning the data populates it in useAsync's `data` value
          return pokemonData
        }),
      )
    }
